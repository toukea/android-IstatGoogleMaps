...
protected Handler ZommerHandler=new Handler(){
	public void handleMessage(Message msg) {onZoomComplete(msg);};
};
public GmapZoomer(Context context, GoogleMap map){
	this.context=context;this.mMap=map;
...
public ZoomState obteinewZoomState(int wait,float zoom,float lat,float lng){
...
public void stopZoomTask(){
	...
	try{mMap.stopAnimation();}catch(Exception e){}
}
...
/**
 * <h1>ZoomState<h1>
 * this class represent a Zoom state that can be add to the GmapZoomer in order tho perform a Google map Zoom.
 * @author Toukea tatsi jephte (Istat)
 */
public  class ZoomState{
		...
		handler=new Handler(){
			...
			public void handleMessage(Message msg) {
				...
				if(point!=null && mMap!=null){
					if(zoomTime<0)
					mMap.animateCamera(CameraUpdateFactory.newCameraPosition(getCameraUpdate()),callback);
					else
					mMap.animateCamera(CameraUpdateFactory.newCameraPosition(getCameraUpdate()),zoomTime,callback);
	...
	public ZoomState setTitl(float titl) {
		if(titl>90) titl=90;else if(titl<0) titl=0;
		this.titl = titl;
	 ...
	 public ZoomStateList setNextZoom(final ZoomState state){
		 callback=new CancelableCallback() {
			...
			//	if(callback!=null)callback.onFinish();
				...
				//Toast.makeText(context, "calback:"+zoom, Toast.LENGTH_LONG).show();
			}
			
			@Override
			public void onCancel() {
				// TODO Auto-generated method stub
				if(state.callback!=null)state.callback.onCancel();
			}
		};
